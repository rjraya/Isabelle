...
lemma dichotomy_2:
  ...
  shows "(x2,y2) = i (x1,y1)"
proof -
  have "(x1,y1) \<in> e_aff" "(x2,y2) \<in> e_aff"
    using assms unfolding e_aff_0_def by blast+
  then have "e' x1 y1 = 0" "e' x2 y2 = 0"
    using e_aff_def by blast+
  {assume "x1 = 0 \<or> y1 = 0 \<or> x2 = 0 \<or> y2 = 0" 
  then have ?thesis
    using assms unfolding e_aff_0_def e_aff_def e'_def delta_def delta_minus_def delta_plus_def
    apply(simp)
  ...
  define e1 where "e1 = e' x1 y1"
  define e2 where "e2 = e' x2 y2"
  define q where "q = 1 / (x1 * y1 * x2 * y2)"
  define v0y where "v0y = x1 * y2 + y1 * x2" 
  define m0x where "m0x = (x1 * x2 - y1 * y2) - (1 - d * x1 * y1 * x2 * y2)"
  have m0x_0: "m0x = 0"
    using assms(1) unfolding m0x_def by(simp,simp add: c_eq_1)
  assume as: "x1 \<noteq> 0" "y1 \<noteq> 0" "x2 \<noteq> 0" "y2 \<noteq> 0"  
  have d_expr: "d = (y1*y2 - x1*x2 + 1) / (x1*y1*x2*y2)"
               "d = (x1^2 + y1^2 - 1) / (x1^2 * y1^2)"
               "d = (x2^2 + y2^2 - 1) / (x2^2 * y2^2)"
    using m0x_0 unfolding m0x_def apply(simp add: divide_simps as)
    using assms(2) unfolding e_aff_0_def e_aff_def e'_def apply(simp add: d_nz as t_expr)
    using assms(2) unfolding e_aff_0_def e_aff_def e'_def by(simp add: d_nz as t_expr)
      

  have ?thesis
    apply(simp)
    apply(rule conjI)
    using assms(2) unfolding e_aff_0_def e_aff_def e'_def delta_def delta_minus_def delta_plus_def
     apply(simp add: t_expr d_expr as divide_simps) 
    thm d_expr(2)
    using [[unify_trace_failure]]
    apply(subst (1) d_expr(2))
    apply(simp add: d_expr as)
     apply(simp add: algebra_simps divide_simps as) 
    
  have "\<exists> q1 q2 q3 q4. x1 - x2 = q1 * e1 + q2 * e2 + q3 * (q * x1 * y1 * x2 * y2 - 1) +
                                 q3 * v0y + q4 * m0x"
    unfolding e1_def e'_def e2_def v0y_def m0x_def q_def
    apply(simp add: divide_simps as t_expr) 
...
lemma b_cc_case':
  assumes closure_lem: "ext_add (x, y) (\<tau> (x', y')) \<in> e_aff"
  assumes x_y_aff: "(x,y) \<in> e_aff" "\<tau> (x', y') \<in> e_aff" "\<tau> (x', y') \<in> e_circ"
  assumes cc: "x' \<noteq> 0" "y' \<noteq> 0"
  assumes b: "\<not> ((x, y) \<in> e_circ \<and> (\<exists>g\<in>symmetries. (x', y') = (g \<circ> i) (x, y)))"
  shows "
  ...
  (\<exists>g\<in>symmetries. (x', y') = (g \<circ> i) (x, y))"
proof -
  assume as: "fst (ext_add (x,y) (\<tau> (x',y'))) = 0 \<or> snd (ext_add (x,y) (\<tau> (x',y'))) = 0"
  define r1 where "r1 = fst(ext_add (x,y) (\<tau> (x',y')))"
  define r2 where "r2 = snd(ext_add (x,y) (\<tau> (x',y')))"
  from closure_lem have "(r1,r2) \<in> e_aff" using r1_def r2_def by simp
  have cases: "r1 = 0 \<or> r2 = 0" 
    using as r1_def r2_def by presburger
  {assume "r1 = 0" 
  ...
  have d_eq: "delta_plus x y x y \<noteq> 0"
      ...
      apply(subst power2_eq_square[symmetric])
      using mult_nonneg_nonneg[OF c_d_pos zero_le_power2[of "x*y"]] by auto
  from r_expr have "ext_add (x,y) (\<tau> (x',y')) = g (1,0)" 
    using r1_def r2_def by simp
  also have "... = g (ext_add (x,y) (i (x,y)))"
    sorry
  also have "... = ext_add (x,y) ((g \<circ> i) (x,y))"
    using \<open>g \<in> rotations\<close> unfolding rotations_def
    apply(auto)
    apply(simp add: c_eq_1)+
    apply(simp add: divide_simps)
    apply(simp add: c_eq_1)+
    by(simp add: algebra_simps divide_simps)
  finally have add_eq: "ext_add (x,y) (\<tau> (x',y')) = ext_add (x,y) ((g \<circ> i) (x,y))"
    by blast
  obtain g' where "g' \<in> rotations" "g \<circ> g' = id"
    using rot_inv[OF \<open>g \<in> rotations\<close>] rot_com[OF \<open>g \<in> rotations\<close>] by auto
  then have "add (x,y) (g' (\<tau> (x',y'))) = add (x,y) (i (x,y))"
...
proof -
  ...
  show ?thesis 
    ...
    using assms(1) unfolding assms(1-2) e_proj_def quotient_def by auto
qed
  ...
  then consider
    ...
    next
      ...
      have nz: "x \<noteq> 0" "y \<noteq> 0" "x' \<noteq> 0" "y' \<noteq> 0" 
        using "4"(1,2) assms(1,2) e_proj_elim_2 p_q_expr(2,4) by blast+
      ...
      have taus: "(\<tau> (x', y')) \<in> e_aff" "(\<tau> (x, y)) \<in> e_aff" "\<tau> (x', y') \<in> e_circ"
        ...
        using "4"(1) assms(1) e_aff_bit_def e_proj_def eq_rel in_quotient_imp_subset apply fastforce 
      ...
      proof(cases)
        ...
        have ex_eq: "proj_add ((x, y), l) (\<tau> (x', y'), l'+1) = 
        ...
        proof -
          ...
          proof(cases)
            ...
            from 1 have "proj_add ((x, y), l) (\<tau> (x', y'), l'+1) = 
                       Some(add (x, y) (\<tau> (x', y')), l+l'+1)"   
              using proj_add.simps \<open>(x,y) \<in> e_aff\<close> \<open>(\<tau> (x', y')) \<in> e_aff\<close> by auto
            have "proj_add (\<tau> (x, y), l+1) ((x', y'), l') = 
                  Some(add (\<tau> (x, y)) (x', y'), l+l'+1)"
              using proj_add.simps pd' \<open>(\<tau> (x, y)) \<in> e_aff\<close> \<open>(x',y') \<in> e_aff\<close>
              by (simp add: p_delta_def)
            then show ?thesis
              using \<open>proj_add ((x, y), l) (\<tau> (x', y'), l' + 1) = Some (add (x, y) (\<tau> (x', y')), l + l' + 1)\<close> inversion_invariance_1 nz(1) nz(2) nz(3) nz(4) by auto
          next
            case 2
            then have pd': "p_delta' (\<tau> (x, y), l) ((x', y'), l' + 1) \<noteq> 0"
            ...
            from 2 have "p_delta (\<tau> (x, y), l) ((x', y'), l' + 1) = 0" 
              unfolding p_delta_def delta_def delta_plus_def delta_minus_def
              by(simp add: t_nz nz field_simps power2_eq_square[symmetric] t_expr d_nz)
            from this 2 have "proj_add ((x, y), l) (\<tau> (x', y'), l'+1) = 
                       Some(ext_add (x, y) (\<tau> (x', y')), l+l'+1)"   
            ...
            then show ?thesis 
              using \<open>proj_add ((x, y), l) (\<tau> (x', y'), l' + 1) = Some (ext_add (x, y) (\<tau> (x', y')), l + l' + 1)\<close> inversion_invariance_2 nz(1) nz(2) nz(3) nz(4) by presburger
          next
            case 3
            ...
            from 3 have pd': "p_delta' (\<tau> (x, y), l) ((x', y'), l' + 1) = 0"
              unfolding p_delta'_def delta'_def delta_x_def delta_y_def
              by(simp add: t_nz nz field_simps power2_eq_square[symmetric] t_expr d_nz)
            from 3 have "proj_add ((x, y), l) (\<tau> (x', y'), l' + 1) = None"
              by (simp add: proj_add.simps)
            then show ?thesis 
              using \<open>p_delta (\<tau> (x, y), l) ((x', y'), l' + 1) = 0\<close> p_delta'_def p_delta_def pd' proj_add.simps by auto
          qed
        qed
        have dom_ne: "(dom (\<lambda>(x, y). proj_add x y) \<inter> {((x, y), l), (\<tau> (x, y), l + 1)} \<times> {((x', y'), l'), (\<tau> (x', y'), l' + 1)}) \<noteq> {}"
          using covering[OF assms] unfolding 4 proj_add_class_def by blast     
      ...
      {(((x, y), l), (x', y'), l'), (((x, y), l), \<tau> (x', y'), l' + 1),
       ...
       ((\<tau> (x, y), l + 1), \<tau> (x', y'), l' + 1)}) = 
          ...
          have "card ({(add (x, y) (x', y'), l + l'), (add (x, y) (\<tau> (x', y')), l + l' + 1)} // gluing) = Suc 0"
          ...
          next
            ...
            have "gluing `` {(add (x, y) (x', y'), l + l')} = 
                   {(add (x,y) (x',y'),l+l'),((add (x,y) (\<tau> (x',y'))),l+l'+1)}"
              by (smt "4"(2) \<open>(ext_add (x, y) (x', y'), l + l') = (add (x, y) (x', y'), l + l')\<close> add.commute add_cancel_right_left add_ext_add[OF nz] add_nz(1) add_nz(2) assms(2) bit_not_1_iff closure_lem doubleton_eq_iff gluing_class gluing_inv inversion_invariance_1[OF nz] nz(1) nz(2) nz(3) p_q_expr(4) prod.collapse)
            then have "{(add (x, y) (x', y'), l + l'), (add (x, y) (\<tau> (x', y')), l + l' + 1)} // gluing =
                  {gluing `` {(add (x, y) (x', y'), l + l')}}" 
          ...
          then show ?thesis
            unfolding 4 proj_add_class_def
            by(simp del: \<tau>.simps,subst dom_eq,subst add_eq,fastforce)
        next
          case bbb
            from bbb have v3: 
          ...
          {(add (x, y) (x', y'), l + l'),(ext_add (x, y) (\<tau> (x', y')), l+l'+1)}"
            ...
            proof -
              ...
              then show ?thesis 
                unfolding e_aff_def using e_e'_iff z3_d z3_def z2_d by simp
            qed        
            have "ext_add (x, y) (\<tau> (x', y')) \<noteq> 0"
            ...
            then have "gluing `` {(ext_add (x, y) (\<tau> (x', y')), l + l' + 1)} = {(add (x, y) (x', y'), l + l'), (ext_add (x, y) (\<tau> (x', y')), l + l' + 1)}"
              using gluing_cases_explicit e_proj_elim_2 sorry
            show ?thesis
              unfolding 4 proj_add_class_def
      ...
      {(((x, y), l), (x', y'), l'), ((\<tau> (x, y), l + 1), \<tau> (x', y'), l' + 1)}) = 
            ...
            have "(add (x, y) (x', y'), l + l') = (add (\<tau> (x, y)) (\<tau> (x', y')), l + l')"
              using v1_eq_v2 by blast
            show ?thesis 
              unfolding 4 proj_add_class_def 
              apply(subst dom_eq,subst add_eq)
              by (metis One_nat_def \<open>(add (x, y) (x', y'), l + l') = (add (\<tau> (x, y)) (\<tau> (x', y')), l + l')\<close> card_empty card_insert_if empty_iff finite.intros(1) insert_absorb2 singleton_quotient)
          qed
      next
        case c
        then have ld_nz: "delta' x y x' y' \<noteq> 0" 
          ...
          have "delta x y x' y' = 0" 
            ...
            unfolding e_aff_0_def by force
          have "(x',y') \<in> e_circ"
            unfolding e_circ_def using nz \<open>(x',y') \<in> e_aff\<close> by blast
          then have "\<tau> (x', y') \<in> e_circ" 
            using nz \<tau>_circ by blast
          then have "\<tau> (x', y') \<in> e_aff"
            unfolding e_circ_def by force
          have v1: "proj_add ((x, y), l) ((x', y'), l') = Some (ext_add (x, y) (x', y'), l + l')"
            by(simp add: proj_add.simps p_delta'_def p_delta_def \<open>(x,y) \<in> e_aff\<close> \<open>(x',y') \<in> e_aff\<close> ld_nz \<open>delta x y x' y' = 0\<close>) 
...
proof -
  ...
  define x1'_add where "x1'_add = fst(add (x1, y1) (x2, y2))"
  define y1'_add where "y1'_add = snd(add (x1, y1) (x2, y2))"
  define x2'_add where "x2'_add = fst(add (x2, y2) (x3, y3))"
  define y2'_add where "y2'_add = snd(add (x2, y2) (x3, y3))"

  define add_fst_eq where
    "add_fst_eq = -(t^2*x2*y2*(-x2*x3*y1*x1^2+x1^2*x2*x3^3*y1+x1*x3*y1^2*y2-t^2*x1^3*x2^2*x3*y1^2*y2-x1*x3^3*y1^2*y2+t^2*x1^3*x2^2*x3^3*y1^2*y2+t^2*x1^2*x2*x3*y1^3*y2^2-t^2*x1^2*x2*x3^3*y1^3*y2^2+x1*x2*x3^2*y3-x1^3*x2*x3^2*y3-x1*x2*y1^2*y3+t^2*x1^3*x2^3*x3^2*y1^2*y3-x1^2*y1*y2*y3+x3^2*y1*y2*y3+t^2*x1^2*x2^2*y1^3*y2*y3-x3^2*y1^3*y2*y3+t^2*x1^3*x2*y1^2*y2^2*y3-t^4*x1^3*x2^3*x3^2*y1^2*y2^2*y3+t^2*x1^2*x3^2*y1^3*y2^3*y3-t^4*x1^2*x2^2*x3^2*y1^3*y2^3*y3+x2*x3*y1*y3^2-t^2*x1^2*x2^3*x3^3*y1*y3^2-x2*x3*y1^3*y3^2+t^2*x1^2*x2^3*x3*y1^3*y3^2-x1*x3*y2*y3^2+x1^3*x3*y2*y3^2+t^2*x1*x2^2*x3^3*y2*y3^2-t^2*x1^3*x2^2*x3^3*y2*y3^2-t^2*x2*x3^3*y1*y2^2*y3^2+t^4*x1^2*x2^3*x3^3*y1*y2^2*y3^2-t^4*x1^2*x2^3*x3*y1^3*y2^2*y3^2+t^2*x2*x3^3*y1^3*y2^2*y3^2-t^2*x1^3*x3*y1^2*y2^3*y3^2+t^4*x1^3*x2^2*x3*y1^2*y2^3*y3^2+t^2*x1*x3^3*y1^2*y2^3*y3^2-t^4*x1*x2^2*x3^3*y1^2*y2^3*y3^2+x1*x2*y1^2*y3^3-t^2*x1*x2^3*x3^2*y1^2*y3^3+x1^2*y1*y2*y3^3-t^2*x2^2*x3^2*y1*y2*y3^3-t^2*x1^2*x2^2*y1^3*y2*y3^3+t^2*x2^2*x3^2*y1^3*y2*y3^3-t^2*x1*x2*x3^2*y2^2*y3^3+t^2*x1^3*x2*x3^2*y2^2*y3^3-t^2*x1^3*x2*y1^2*y2^2*y3^3+t^4*x1*x2^3*x3^2*y1^2*y2^2*y3^3-t^2*x1^2*x3^2*y1*y2^3*y3^3+t^4*x1^2*x2^2*x3^2*y1*y2^3*y3^3)/(-1+t^4*x1^2*x2^2*y1^2*y2^2+t^2*x1*x2^2*x3*y1*y3+t^2*x1^2*x2*x3*y2*y3-t^2*x2*x3*y1^2*y2*y3-t^2*x1*x3*y1*y2^2*y3)*(-1+t^2*x1*x2*x3^2*y1*y2+t^2*x1*x2^2*x3*y1*y3-t^2*x1*x3*y1*y2^2*y3-t^2*x1*x2*y1*y2*y3^2+t^4*x2^2*x3^2*y2^2*y3^2))"

  have "add_fst_eq = fst(add (add (x1,y1) (x2,y2)) (x3,y3)) - fst(add (x1,y1) (add (x2,y2) (x3,y3)))"
    unfolding add_fst_eq_def 
    apply(simp)
    apply(simp add: t_expr c_eq_1)
    apply(simp add:  divide_simps) 
    using assms unfolding e_aff_def e'_def
    apply(simp add: t_expr c_eq_1,safe)
                        defer 1
    apply(simp add: t_expr c_eq_1)
    apply(simp add: algebra_simps)
    sledgehammer
    
  assume as: "delta_plus x1 y1 x2 y2 \<noteq> 0" "delta_minus x1 y1 x2 y2 \<noteq> 0"
         "delta_plus x2 y2 x3 y3 \<noteq> 0" "delta_minus x2 y2 x3 y3 \<noteq> 0"
         "delta_plus (fst (add (x1,y1) (x2,y2))) (snd (add (x1,y1) (x2,y2))) x3 y3 \<noteq> 0"
         "delta_minus (fst (add (x1,y1) (x2,y2))) (snd (add (x1,y1) (x2,y2))) x3 y3 \<noteq> 0"
         "delta_plus x1 y1 (fst (add (x2,y2) (x3,y3))) (fst (add (x2,y2) (x3,y3))) \<noteq> 0"
  then have as1: "1 + d * x1 * y1 * x2 * y2 \<noteq> 0"
            "d * x1 * y1 * x2 * y2 \<noteq> 1"
            "1 + d * x2 * y2 * x3 * y3 \<noteq> 0"
            "d * x2 * y2 * x3 * y3 \<noteq> 1"
            "1 + d * fst (add (x1, y1) (x2, y2)) * snd (add (x1, y1) (x2, y2)) * x3 * y3 \<noteq> 0"
    "1 - d * fst (add (x1, y1) (x2, y2)) * snd (add (x1, y1) (x2, y2)) * x3 * y3 \<noteq> 0"
    "1 + d * x1 * y1 * fst (add (x2, y2) (x3, y3)) * fst (add (x2, y2) (x3, y3)) \<noteq> 0"
    unfolding delta_def delta_plus_def delta_minus_def 
    by(auto)
  have "fst(add (add (x1,y1) (x2,y2)) (x3,y3)) - fst(add (x1,y1) (add (x2,y2) (x3,y3))) = 1"
    apply(simp)
    
  have "(\<exists> q1 q2 q3.
              fst(add (add (x1,y1) (x2,y2)) (x3,y3)) - fst(add (x1,y1) (add (x2,y2) (x3,y3))) =
              q1 * e1 + q2 * e2 + q3 * e3)"
    unfolding delta_def delta_plus_def delta_minus_def e_aff_def e'_def e1_def e2_def e3_def
    apply(simp)
    apply(simp add: c_eq_1 t_expr)
    apply(simp add: divide_simps d_nz power2_eq_square as1) 
    apply(simp add: as1 )
    apply(simp add: algebra_simps as1)
    try0
qed
