...
Proof_Context.get_fact;
Proof_Context.note_thmss;
Proof_Context.add_facts
\<close>

ML \<open>
val x1'_expr = Goal.prove ctxt [] (map Thm.full_prop_of assms)
                               @{prop "x1' = fst (ext_add (x1,y1) (x2,y2))"}
                          (fn _ => asm_full_simp_tac  @{context} 1) ;
