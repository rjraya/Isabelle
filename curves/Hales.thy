  ...
  then consider
    ...
    next
      ...
      {(((x, y), l), (x', y'), l'), ((\<tau> (x, y), l + 1), \<tau> (x', y'), l' + 1)}) = 
              ...
              using v1 v2 by force
            have "(ext_add (x, y) (x', y'), l + l') = (ext_add (\<tau> (x, y)) (\<tau> (x', y')), l + l')"
            proof -
            ...
            show ?thesis 
              unfolding 4 proj_add_class_def 
              apply(subst dom_eq,subst add_eq)
              by (metis One_nat_def \<open>(ext_add (x, y) (x', y'), l + l') = (ext_add (\<tau> (x, y)) (\<tau> (x', y')), l + l')\<close> card_empty card_insert_if empty_iff finite.intros(1) insert_absorb2 singleton_quotient)
          qed
      qed    
    qed
  qed 
...
proof(unfold_locales,simp_all)
  ...
  show "\<And>x y. x \<in> e_proj \<Longrightarrow> y \<in> e_proj \<Longrightarrow> proj_addition x y \<in> e_proj"
    using well_defined unfolding proj_addition_def 
qed
