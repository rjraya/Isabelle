...
local_setup \<open>
  Local_Theory.note ((@{binding "second_assoc"}, []), [concrete_assoc "ext" "ext" "ext" "add"]) #> snd 
\<close>
...
proof - 
  ...
  \<close>)+
    ...
    apply(tactic \<open>REPEAT (EqSubst.eqsubst_tac @{context} [0] 
              @{thms left_diff_distrib x1'_expr y1'_expr x3'_expr y3'_expr delta_x_def delta_y_def delta_minus_def delta_plus_def} 1)\<close>)
    apply(tactic \<open>simp_tac @{context} 1\<close>)    
    apply(tactic \<open>
   let 
    val pat = [Rewrite.In,Rewrite.Term (@{const divide('a)} $ Var (("c", 0), \<^typ>\<open>'a\<close>) $ 
...
proof -
  ...
  then have "gxpoly = 0" 
    using e1_def assms(13-15) e2_def e3_def by auto
  moreover have "Delta\<^sub>x \<noteq> 0" 
    using Delta\<^sub>x_def delta'_def delta_def assms(7-11) non_unfolded_adds by auto
  ultimately have "g\<^sub>x = 0" 
    using  gxpoly_def by auto
...
proof -
  ...
  have simp1gx: "
    ...
    ((x2 * x3 - y2 * y3) * y1 * delta_plus x2 y2 x3 y3 -
     x1 * (x2 * y3 + y2 * x3) * delta_minus x2 y2 x3 y3)
  "
    apply(subst x1'_expr y1'_expr x3'_expr y3'_expr)+
    apply(subst delta_x_def)
    apply(rewrite at "_ / \<hole>" delta_minus_def[symmetric] delta_plus_def[symmetric])+
    by(simp add: divide_simps assms(5-8) c_eq_1 delta'_def delta_def) 
  ...
  have simp2gx:
    ...
    apply(subst delta_minus_def)
    apply(rewrite at "_ / \<hole>" delta_minus_def[symmetric] delta_plus_def[symmetric])+
    by(simp add: divide_simps assms(5-8) delta'_def delta_def)

  have "\<exists> r1 r2 r3. g\<^sub>x * Delta\<^sub>x = r1 * e1 + r2 * e2 + r3 * e3"
    unfolding g\<^sub>x_def Delta\<^sub>x_def e1_def e2_def e3_def
    apply(tactic \<open>asm_full_simp_tac (@{context} addsimps 
                                @{thms g\<^sub>x_def  Delta\<^sub>x_def assms(1) assms(2)}) 1\<close>)
    apply(tactic \<open>
   let 
    ...
    val to = NONE
   in
    CCONVERSION (Rewrite.rewrite_conv @{context} (pat, to) @{thms delta_x_def[symmetric] delta_minus_def[symmetric]}) 1 
   end
  \<close>)+
    apply(tactic \<open>asm_full_simp_tac (@{context}
                   addsimps (@{thms divide_simps assms(9-11)})) 1\<close>)  
...
proof -
  ...
  have "\<exists> r1 r2 r3. g\<^sub>x * Delta\<^sub>x = r1 * e1 + r2 * e2 + r3 * e3"
    unfolding e1_def e2_def e3_def
    apply(tactic \<open>asm_full_simp_tac (@{context} addsimps 
                                @{thms g\<^sub>x_def  Delta\<^sub>x_def assms(1) assms(2)}) 1\<close>)
    apply(tactic \<open>
   let 
...
proof -
  ...
  have "\<exists> r1 r2 r3. g\<^sub>y * Delta\<^sub>y = r1 * e1 + r2 * e2 + r3 * e3"
    unfolding g\<^sub>y_def Delta\<^sub>y_def 
    apply(simp add: assms(1,2))
    apply(rewrite at "_ / \<hole>" delta_y_def[symmetric] delta_plus_def[symmetric])+
    apply(simp add: divide_simps assms)
    apply(subst left_diff_distrib)
    apply(simp add: simp1gy simp2gy)
    unfolding delta_x_def delta_y_def 
...
proof -
  ...
  have "\<exists> r1 r2 r3. g\<^sub>y * Delta\<^sub>y = r1 * e1 + r2 * e2 + r3 * e3"
    unfolding g\<^sub>y_def Delta\<^sub>y_def 
    apply(simp add: assms(1,2))
    apply(rewrite at "_ / \<hole>" delta_plus_def[symmetric] delta_y_def[symmetric])+
    apply(subst (3) delta_y_def[symmetric])
    apply(simp add: divide_simps assms)
    apply(subst left_diff_distrib)
    apply(simp add: simp1gy simp2gy)
