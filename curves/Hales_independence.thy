...
lemma gluing_aff:
  assumes "((p0,l),(p1,j)) \<in> gluing"
  shows "p0 \<in> e'_aff" "p1 \<in> e'_aff"
proof -
  obtain x0 y0 x1 y1 where "p0 = (x0,y0)" "p1 = (x1,y1)"
    by fastforce+
  then show "p0 \<in> e'_aff" "p1 \<in> e'_aff"
...
proof
  ...
  have eq: "\<langle>((x,y),l)\<rangle> = {((x,y),l)}"
    (is "_ = ?B")
...
next
  assume "x = 0 \<or> y = 0"
  then have eq: "\<langle>((x,y),l)\<rangle> = {((x,y),l)}"
...
proof 
  ...
  then have eq: "\<langle>((x,y),l)\<rangle> = {((x,y),l),(\<tau> (x,y),l+1)}"
...
next
  ...
  have eq: "\<langle>((x,y),l)\<rangle> = {((x,y),l),(\<tau> (x,y),l+1)}"
    (is "_ = ?B")
   using quotientI[of _ ?B gluing] eq_class_simp as by auto
...
lemma e_proj_aff:
  "\<langle>((x,y),l)\<rangle> \<in> e_proj \<longleftrightarrow> (x,y) \<in> e'_aff"
proof 
  assume "\<langle>((x,y),l)\<rangle> \<in> e_proj"
...
next
  ...
  show "\<langle>((x,y),l)\<rangle> \<in> e_proj"
...
definition proj_addition (infix "\<oplus>" 50) where 
...
lemma covering_with_deltas:
  assumes "(\<langle>((x,y),l)\<rangle>) \<in> e_proj" "(\<langle>((x',y'),l')\<rangle>) \<in> e_proj"
...
(*
corollary plus_expr:
  assumes "\<langle>(p,l)\<rangle> \<in> e_proj" "\<langle>(q,l')\<rangle> \<in> e_proj"
  shows "\<langle>(p,l)\<rangle> \<oplus> \<langle>(q,l')\<rangle> = \<langle>proj_add (p,l) (q,l')\<rangle>"
...
lemma gluing_add_4: 
  assumes "\<langle>((x,y),l)\<rangle> = {((x, y), l), (\<tau> (x, y), l + 1)}" 
          ...
          "\<langle>((x,y),l)\<rangle> \<in> e_proj" "gluing `` {((x', y'), l')} \<in> e_proj" "delta x y x' y' \<noteq> 0"
  shows "\<langle>((x,y),l)\<rangle> \<oplus> gluing `` {((x', y'), l')} = 
...
lemma gluing_add:
  ...
  shows "gluing `` {((x1,y1),l)} \<oplus> gluing `` {((x2,y2),j)} = 
...
proof -
  ...
  have "delta x1 y1 x2 y2 \<noteq> 0 \<or> delta' x1 y1 x2 y2 \<noteq> 0 \<or>
    ...
    using assms covering_with_deltas by auto
  ...
  next
    ...
    then have nt_in_class: "\<not>(((x1,y1), x2, y2) \<in> e'_aff_0 \<or> ((x1,y1), x2, y2) \<in> e'_aff_1)"
                      "\<not>((\<tau> (x1,y1), \<tau> (x2,y2)) \<in> e'_aff_0 \<or> (\<tau> (x1,y1), \<tau> (x2,y2)) \<in> e'_aff_1)"
      unfolding e'_aff_0_def e'_aff_1_def
    ...
    proof(rule ccontr)
      ...
      then have "proj_add_class \<langle>((x1,y1),l1)\<rangle> \<langle>((x2,y2),l2)\<rangle> = {}"
        ...
        unfolding e'_aff_0_def e'_aff_1_def apply simp
        using c by blast
      then show "False"
    ...
    proof(cases)
      ...
      have "delta x1 y1 (fst (\<tau> (x2,y2))) (snd (\<tau> (x2,y2))) = 0"
        ...
        subgoal
          using c(1) \<open>x2 = 0 \<or> y2 = 0\<close> c_eq_1
          unfolding delta_def delta_plus_def delta_minus_def 
                    delta'_def delta_x_def delta_y_def
          by fastforce
        subgoal
      ...
      have eq: "{proj_add ((x1a, y1a), i) ((x2, y2), l2) |x1a y1a i.
                 ...
                 \<and> (((x1a, y1a), x2, y2) \<in> e'_aff_0 \<or> ((x1a, y1a), x2, y2) \<in> e'_aff_1)} = 
      ...
      have "proj_add_class ?c1 ?c2 =
             {proj_add ((x1a, y1a), i) ((x2, y2), l2) |x1a y1a i. 
                 (((x1a,y1a),i) = ((x1,y1),l1) \<or> ((x1a,y1a),i) = (\<tau> (x1, y1),l1+1))
                 \<and> (((x1a, y1a), x2, y2) \<in> e'_aff_0 \<or> ((x1a, y1a), x2, y2) \<in> e'_aff_1)} // gluing"
    ...
    next
      ...
      have nz: "x2 \<noteq> 0" "y2 \<noteq> 0" "x1 = 0 \<or> y1 = 0"
        using bb assms e_proj_elim_1 e_proj_elim_2 in_aff by auto
      ...
      have eq: "{proj_add ((x1,y1),l1) ((x1a,y1a),i) |x1a y1a i. 
                 ...
                 \<and> (((x1,y1), (x1a,y1a)) \<in> e'_aff_0 \<or> ((x1,y1), (x1a,y1a)) \<in> e'_aff_1)} = 
      ...
      have "proj_add_class ?c1 ?c2 =
             {proj_add ((x1,y1),l1) ((x1a,y1a),i) |x1a y1a i. 
                 (((x1a,y1a),i) = ((x2,y2),l2) \<or> ((x1a,y1a),i) = (\<tau> (x2, y2),l2+1))
                 \<and> (((x1,y1), (x1a,y1a)) \<in> e'_aff_0 \<or> ((x1,y1), (x1a,y1a)) \<in> e'_aff_1)} // gluing"
    ...
    next
      case cc
       have nz: "x2 \<noteq> 0" "y2 \<noteq> 0" "x1 \<noteq> 0" "y1 \<noteq> 0"
         using cc assms e_proj_elim_1 e_proj_elim_2 in_aff by auto
      ...
      proof -
        ...
        then show ?thesis
          by (metis (no_types) \<open>delta (fst (\<tau> (x1, y1))) (snd (\<tau> (x1, y1))) x2 y2 \<noteq> 0\<close> assms(2) c(3) ext_curve_addition.e_proj_aff ext_curve_addition_axioms gluing_inv in_aff(1) inversion_invariance_1 nz(1) nz(2) nz(3) nz(4) prod.collapse proj_add.simps(1))
      qed
      ...
      have eq: "{proj_add ((x, y), i) ((x', y'), j) | 
              ...
              ((x, y), i) \<in> ?c1 \<and> 
              ((x', y'), j) \<in> ?c2 \<and> 
              ((x, y), (x', y')) \<in> e'_aff_0 \<union> e'_aff_1} = 
            {proj_add ((x1, y1), l1) (\<tau> (x2, y2), l2+1)}"
         (is "?s1 = ?s2")
      proof(intro equalityI;standard)
        fix s
      ...
      next 
        ...
        then show "s \<in> ?s1"
        proof -
          have "((x1,y1),\<tau> (x2,y2)) \<in> e'_aff_0"
            unfolding e'_aff_0_def apply(simp del: \<tau>.simps)
          ...
          then show ?thesis
            apply(simp del: \<tau>.simps)
            using \<open>((x1, y1), \<tau> (x2, y2)) \<in> e'_aff_0\<close> \<open>s = proj_add ((x1, y1), l1) (\<tau> (x2, y2), l2 + 1)\<close> cc(1) by fastforce
        qed
      qed     
      ...
      then show ?thesis 
        by (simp add: \<open>(gluing `` {((x1, y1), l1)} \<oplus> gluing `` {((x2, y2), l2)}) = gluing `` {proj_add ((x1, y1), l1) (\<tau> (x2, y2), l2 + 1)}\<close>)
    qed    
  next
    case d
    have "delta (fst (\<tau> (x1,y1))) (snd (\<tau> (x1,y1))) (fst (\<tau> (x2,y2))) (snd (\<tau> (x2,y2))) = 0"
    ...
    proof(cases)
      ...
      have "delta x1 y1 (fst (\<tau> (x2,y2))) (snd (\<tau> (x2,y2))) = 0"
           "delta' x1 y1 (fst (\<tau> (x2,y2))) (snd (\<tau> (x2,y2))) = 0"
        subgoal
          using d(1) \<open>x2 = 0 \<or> y2 = 0\<close> c_eq_1
          unfolding delta_def delta_plus_def delta_minus_def 
                    delta'_def delta_x_def delta_y_def
          by fastforce
        subgoal
    ...
    next
      ...
      proof -
        have "l1 + (l2 + 1) = l1 + 1 + l2" by simp
        then show ?thesis
          by (metis \<open>delta' (fst (\<tau> (x1, y1))) (snd (\<tau> (x1, y1))) x2 y2 \<noteq> 0\<close> assms(1) assms(2) d(3) e_proj_aff gluing_inv inversion_invariance_2 nz(1) nz(2) nz(3) nz(4) prod.collapse proj_add.simps(2))
      qed
      ...
      next 
        ...
        then show "s \<in> ?s1"
        proof -
          have "((x1,y1),\<tau> (x2,y2)) \<in> e'_aff_1"
            unfolding e'_aff_1_def apply(simp del: \<tau>.simps)
            by (metis (mono_tags, lifting) assms(2) d(3) e_proj_aff gluing_inv in_aff(1) nz(1) nz(2) prod.case_eq_if prod.collapse)
          have "(((x2, y2), l2), (\<tau> (x2, y2), l2+1)) \<in> gluing"
            using cc(2) by auto
          then show ?thesis
            apply(simp del: \<tau>.simps)
            using \<open>((x1, y1), \<tau> (x2, y2)) \<in> e'_aff_1\<close> \<open>s = proj_add ((x1, y1), l1) (\<tau> (x2, y2), l2 + 1)\<close> cc(1) by fastforce
        qed
      qed     
...
proof -
  ...
  proof(cases)
    ...
    have "proj_addition p q = proj_addition (\<langle>((x,y),l)\<rangle>) (gluing `` {((x', y'), l')})"
...
lemma proj_add_class_inv:
  ...
  shows "proj_addition (\<langle>(p,l)\<rangle>) (gluing `` {(i p,l')}) = {((1, 0), l+l')}"
...
proof -
  ...
  have "proj_addition {((1, 0), 0)} x = 
        proj_addition (\<langle>((1, 0), 0)\<rangle>) (gluing `` {((x0,y0),l0)})"
...
proof -
  ...
  have eq: "\<langle>((x,y),l)\<rangle> = {((x, y), l)} \<or> 
    ...
    using assms gluing_cases_explicit p_eq by auto
  from eq consider  
    (1) "\<langle>((x,y),l)\<rangle> = {((x, y), l)}" | 
    (2) "\<langle>((x,y),l)\<rangle> = {((x, y), l), (\<tau> (x, y), l+1)}"
    by fast
  then show "tf \<rho> (\<langle>(p,l)\<rangle>) \<in> e_proj"
  proof(cases)
    case 1
    have zeros: "x = 0 \<or> y = 0"
      using in_aff e_proj_elim_1 assms e_proj_aff 1 p_eq by auto
    show ?thesis 
      unfolding tf_def
      using rho_aff zeros e_proj_elim_1 1 p_eq by auto
  next
    case 2
    have zeros: "x \<noteq> 0" "y \<noteq> 0"
      using in_aff e_proj_elim_2 assms e_proj_aff 2 p_eq by auto
    show ?thesis 
      unfolding tf_def
      using rho_aff zeros e_proj_elim_2 2 p_eq by fastforce
  qed
qed

lemma insert_rho_gluing:
  assumes "\<langle>((x,y),l)\<rangle> \<in> e_proj"
  shows "tf \<rho> (\<langle>((x,y),l)\<rangle>) = gluing `` {(\<rho> (x, y), l)}"
proof -
  have in_aff: "(x,y) \<in> e'_aff" 
      using assms e_proj_aff by blast
  have rho_aff: "\<rho> (x,y) \<in> e'_aff" 
  ...
  from eq consider  
    ...
    (2) "\<langle>((x,y),l)\<rangle> = {((x, y), l), (\<tau> (x, y), l+1)}"
    by fast
  then show "tf \<rho> (\<langle>((x,y),l)\<rangle>) = gluing `` {(\<rho> (x, y), l)}"
  proof(cases)
    case 1
    have zeros: "x = 0 \<or> y = 0"
...
lemma insert_rotation_gluing:
  assumes "\<langle>((x,y),l)\<rangle> \<in> e_proj" "r \<in> rotations"
  shows "tf r (\<langle>((x,y),l)\<rangle>) = gluing `` {(r (x, y), l)}"
proof -
...
lemma tf_tau:
  assumes "\<langle>((x,y),l)\<rangle> \<in> e_proj" 
  shows "gluing `` {((x,y),l+1)} = tf' (\<langle>((x,y),l)\<rangle>)"
  using assms unfolding symmetries_def
proof -
  have in_aff: "(x,y) \<in> e'_aff" 
    using e_proj_aff assms by simp

  have gl_expr: "\<langle>((x,y),l)\<rangle> = {((x,y),l)} \<or> 
                 \<langle>((x,y),l)\<rangle> = {((x,y),l),(\<tau> (x,y),l+1)}"
    using assms gluing_cases_explicit by simp

  consider (1) "\<langle>((x,y),l)\<rangle> = {((x,y),l)}" | 
           (2) "\<langle>((x,y),l)\<rangle> = {((x,y),l),(\<tau> (x,y),l+1)}" 
...
lemma tf_preserv_e_proj:
  assumes "\<langle>((x,y),l)\<rangle> \<in> e_proj" 
  shows "tf' (\<langle>((x,y),l)\<rangle>) \<in> e_proj"
...
proof -
  ...
  next
    ...
    show ?thesis 
      unfolding gl_eqs 2 tf_def image_def 
      by force
  qed
qed
...
proof -
  ...
  next
    case 3
    then show ?thesis 
      using remove_rho rho_preserv_e_proj assms insert_rotation_gluing by auto
  next
    case 4
    then show ?thesis 
...
lemma remove_tau:
  assumes "\<langle>((x,y),l)\<rangle> \<in> e_proj" "gluing `` {(\<tau> (x,y),l)} \<in> e_proj"
  shows "gluing `` {(\<tau> (x,y),l)} = tf' (\<langle>((x,y),l)\<rangle>)"
  (is "?gt = tf' ?g")
proof -
  ...
  next
    ...
    then have gl_eq: "\<langle>((x,y),l)\<rangle> = {((x,y),l),(\<tau> (x,y),l+1)}"
...
proof -
  ...
  proof(cases)
    ...
    have e_proj': "\<langle>((x,y),l)\<rangle> \<in> e_proj"
      ...
      using e_proj(2) eq r_expr(1) by auto
    have simp2: "tf \<rho>
     (proj_addition (\<langle>((x,y),l)\<rangle>)
       (gluing `` {((r \<circ> i) (x, y), l'+1)})) = 
      gluing `` {((\<rho> \<circ> r) (1,0),l+l'+1)}"
    ...
    proof(cases "?ds1 \<noteq> 0")    
      ...
      show ?thesis 
        apply(subst eq')
        apply(subst remove_rho[of "fst (r (1,0))" "snd (r (1,0))",
                       simplified prod.collapse])
    ...
    show ?thesis
      unfolding p_q_expr
      unfolding remove_rho[OF e_proj(1)] r_expr eq
      unfolding simp1 simp2 by blast
next
  case b
    then have ds: "delta x y x' y' \<noteq> 0"
      unfolding e'_aff_0_def by auto
    have eq1: "proj_addition (tf \<rho> (\<langle>((x,y),l)\<rangle>))
                        (gluing `` {((x', y'), l')}) = 
    ...
    have eq2: "tf \<rho>
     (proj_addition (\<langle>((x,y),l)\<rangle>) (gluing `` {((x', y'), l')})) =
     gluing `` {(add (\<rho> (x,y)) (x', y'), l+l')}"
      apply(subst gluing_add)
...
proof -
  obtain x y l x' y' l' where p_q_expr: "p = \<langle>((x,y),l)\<rangle>" "p = gluing `` {((x', y'), l')}"
...
proof -
  ...
  proof(cases)
    ...
    have "proj_addition (tf' (\<langle>((x,y),l)\<rangle>))
                        (gluing `` {((x', y'), l')}) = 
    ...
    proof -
      note lem1 = remove_rotations[of "fst (i (x,y))" "snd (i (x,y))" "l'+1",
               OF _ r_expr(2), simplified prod.collapse, OF i_proj,symmetric] 
      show ?thesis
      apply(subst lem1)
    ...
    have "tf' (proj_addition (\<langle>((x,y),l)\<rangle>)
          (gluing `` {((x', y'), l')})) = 
          tf' (proj_addition (\<langle>((x,y),l)\<rangle>)
          (gluing `` {((\<tau> \<circ> r \<circ> i) (x, y), l')}))"     
      (is "?lhs = _")
      using assms(1) p_q_expr(1) tf_tau r_expr by auto
    also have "... =
...
proof -
  obtain x y l x' y' l' where p_q_expr: "p =  \<langle>((x,y),l)\<rangle>" "q =  gluing `` {((x', y'), l')}"
...
proof -
  ...
  show ?thesis
    unfolding tf''_def
    apply(rewrite p_eq)
    apply(subst insert_rotation_gluing[OF p_hyp])
    using rotation_preserv_e_proj[OF p_hyp] tf_preserv_e_proj insert_rotation_gluing[OF p_hyp] 
    by (metis i.cases)
qed
...
lemma proj_add_aff:
  assumes "p \<in> e'_aff" "q \<in> e'_aff"
  shows "fst (proj_add (p,l) (q,l')) \<in> e'_aff"
  (* Do it by cases on proj_add *)
  oops
...
lemma independence:
  assumes "((p1,l1),(p2,l2)) \<in> gluing" "((q1,t1),(q2,t2)) \<in> gluing"
  shows "(proj_add (p1,l1) (q1,t1), proj_add (p2,l2) (q2,t2)) \<in> gluing"
proof -
  obtain p1x p1y p2x p2y q1x q1y q2x q2y 
    where p_q_eqs: "p1 = (p1x,p1y)" "p2 = (p2x,p2y)" "q1 = (q1x,q1y)" "q2 = (q2x,q2y)"
    by force
  obtain s1x s1y s2x s2y s1 s2
    where sum_eqs: "proj_add (p1,l1) (q1,t1) = ((s1x,s1y),s1)" "proj_add (p2,l2) (q2,t2) = ((s2x,s2y),s2)"
    by (metis surj_pair)
  have in_aff: "p1 \<in> e'_aff" "p2 \<in> e'_aff" "q1 \<in> e'_aff"  "q2 \<in> e'_aff" 
    using gluing_aff assms by auto
  consider
    (a) "p2 = p1" "l2 = l1" 
        "q2 = q1" "t2 = t1" |
    (b) "p2 = p1" "l2 = l1"
        "q2 = \<tau> q1" "t2 = t1 + 1" "fst q1 \<noteq> 0" "snd q1 \<noteq> 0" |
    (c) "p2 = \<tau> p1" "l2 = l1 + 1" "fst p1 \<noteq> 0" "snd p1 \<noteq> 0" 
        "q2 = q1" "t2 = t1" |
    (d) "p2 = \<tau> p1" "l2 = l1 + 1" "fst p1 \<noteq> 0" "snd p1 \<noteq> 0" 
        "q2 = \<tau> q1" "t2 = t1 + 1" "fst q1 \<noteq> 0" "snd q1 \<noteq> 0"
    using gluing_char[OF assms(1)] gluing_char[OF assms(2)] by auto
  then show ?thesis
  proof(cases)
    case a
    then show ?thesis 
      unfolding sum_eqs gluing_def
      apply(simp del: \<tau>.simps)
      apply(intro conjI)
      using proj_add_aff[OF in_aff(1,3), of l1 t1] sum_eqs fst_conv apply force
      using proj_add_aff[OF in_aff(2,4), of l2 t2] sum_eqs fst_conv apply force
      using sum_eqs by auto
  next
    case b
    have nz: "q1x \<noteq> 0" "q1y \<noteq> 0"
      using b p_q_eqs by force+
    (* (p1,l1) \<oplus> (q1,t1) \<sim> (p1,l1) \<oplus> (\<tau> q1, t1+1) given q1 \<noteq> 0*)
    thm dichotomy_1 add_ext_add_2
    assume "delta (fst p1) (snd p1) (fst (\<tau> q1)) (snd (\<tau> q1)) = 0"
           "delta' (fst p1) (snd p1) (fst (\<tau> q1)) (snd (\<tau> q1)) = 0"
    then have "False"
      unfolding delta_def delta_plus_def delta_minus_def 
                delta'_def delta_x_def delta_y_def  using in_aff
      apply simp
      unfolding p_q_eqs e'_aff_def e'_def
      apply(simp add: t_nz nz d_nz divide_simps algebra_simps power2_eq_square[symmetric] t_expr)
     
    then have "proj_add (p2, l2) (q2, t2) = undefined"
      using b(1) b(3) p_q_eqs(1) p_q_eqs(4) by force
    
    have 
    consider
      (1) "delta (fst p1) (snd p1) (fst q1) (snd q1) \<noteq> 0" |
      (2) "delta' (fst p1) (snd p1) (fst q1) (snd q1) \<noteq> 0" |
      (3) "delta (fst p1) (snd p1) (fst q1) (snd q1) = 0"
          "delta' (fst p1) (snd p1) (fst q1) (snd q1) = 0" by auto
    then have "(proj_add (p1, l1) (q1, t1), proj_add (p1, l1) (\<tau> q1, t1+1)) \<in> gluing"
    proof(cases)
      case 1
      have "proj_add (p1, l1) (q1, t1) = (add p1 q1, l1+t1)"
        using 1 in_aff(1) in_aff(3) p_q_eqs(1) p_q_eqs(3) by auto
      thm covering
      have "delta' (fst p1) (snd p1) (fst (\<tau> q1)) (snd (\<tau> q1)) \<noteq> 0 \<or>
            delta' (fst p1) (snd p1) (fst q1) (snd q1) \<noteq> 0"
        using 1 in_aff
        unfolding delta_def delta_plus_def delta_minus_def
                  delta'_def delta_x_def delta_y_def e'_aff_def e'_def
        apply simp
        unfolding p_q_eqs
        apply(simp add: divide_simps t_nz nz t_expr)
        
      have ?thesis
      proof(cases "delta (fst p1) (snd p1) (fst (\<tau> q1)) (snd (\<tau> q1)) \<noteq> 0")
        case True
        then show ?thesis 
          sorry
      next
      ...
      have "delta' (fst p1) (snd p1) (fst (\<tau> q1)) (snd (\<tau> q1)) = 0 \<or>
            delta (fst p1) (snd p1) (fst (\<tau> q1)) (snd (\<tau> q1)) = 0"
  ...
  next
    ...
    consider
      ...
      (3) "delta (fst p1) (snd p1) (fst q1) (snd q1) = 0"
          "delta' (fst p1) (snd p1) (fst q1) (snd q1) = 0" by auto  
    then have "s1x \<noteq> 0" 
    proof(cases)
      case 1  
      then have "proj_add (p1,l1) (q1,t1) = (add p1 q1, l1+t1)"
        using in_aff(1) in_aff(3) p_q_eqs(1) p_q_eqs(3) by auto
      have "delta (fst p1) (snd p1) (fst (\<tau> q1)) (snd (\<tau> q1)) \<noteq> 0"
        sledgehammer
      have "proj_add (\<tau> p1,l1+1) (q1,t1) = (\<tau> (add p1 q1),l1+t1+1)"
  ...
  next
    case d
    consider 
      (a) "delta (fst p1) (snd p1) (fst q1) (snd q1) \<noteq> 0" |
      (b) "delta' (fst p1) (snd p1) (fst q1) (snd q1) \<noteq> 0" |
      (c) "delta (fst p1) (snd p1) (fst q1) (snd q1) = 0"
          "delta' (fst p1) (snd p1) (fst q1) (snd q1) = 0" by auto  
    then have "proj_add (\<tau> p1, l1 + 1) (\<tau> q1, t1 + 1) = 
               proj_add (p1,l1) (q1,t1)" 
    proof(cases)
      case a
      then have "delta (fst (\<tau> p1)) (snd (\<tau> p1)) (fst (\<tau> q1)) (snd (\<tau> q1)) \<noteq> 0"
        using d(1) d(5) in_aff(2) in_aff(4) p_q_eqs(1) p_q_eqs(3) tau_idemp_explicit tau_tau_d by auto
      have 1: "proj_add (p1, l1) (q1, t1) = (add p1 q1, l1+t1)"
                "proj_add (\<tau> p1,l1+1) (\<tau> q1,t1+1) = (add (\<tau> p1) (\<tau> q1), l1+t1)"        
        using a in_aff(1) in_aff(3) p_q_eqs(1) p_q_eqs(3) apply simp
        using \<open>delta (fst (\<tau> p1)) (snd (\<tau> p1)) (fst (\<tau> q1)) (snd (\<tau> q1)) \<noteq> 0\<close> d(1) d(5) in_aff(2) in_aff(4) p_q_eqs(2) p_q_eqs(4) by force
      have 2: "add (\<tau> p1) (\<tau> q1) = add p1 q1"
        by (metis add_ext_add add_ext_add_2 commutativity d(1) d(3) d(4) d(5) d(7) d(8) ext_add_comm fst_conv i.cases p_q_eqs(1) p_q_eqs(3) snd_conv tau_idemp_point)
      then show ?thesis
        using 1 2 by argo
    next
      case b
      then have "delta' (fst (\<tau> p1)) (snd (\<tau> p1)) (fst (\<tau> q1)) (snd (\<tau> q1)) \<noteq> 0"
        using d(1) d(5) fst_conv in_aff(2) in_aff(4) p_q_eqs(1) p_q_eqs(2) p_q_eqs(3) tau_idemp_point tau_tau_d' by force
      have 1: "proj_add (p1, l1) (q1, t1) = (ext_add p1 q1, l1+t1)"
              "proj_add (\<tau> p1,l1+1) (\<tau> q1,t1+1) = (ext_add (\<tau> p1) (\<tau> q1), l1+t1)"        
        using b in_aff(1) in_aff(3) p_q_eqs(1) p_q_eqs(3) apply simp
        using \<open>delta' (fst (\<tau> p1)) (snd (\<tau> p1)) (fst (\<tau> q1)) (snd (\<tau> q1)) \<noteq> 0\<close> d(1) d(5) in_aff(2) in_aff(4) p_q_eqs(2) p_q_eqs(4) by force
      have 2: "ext_add (\<tau> p1) (\<tau> q1) = ext_add p1 q1"
        by (metis add_ext_add add_ext_add_2 commutativity d(1) d(3) d(4) d(5) d(7) d(8) ext_add_comm fst_conv i.cases p_q_eqs(1) p_q_eqs(3) snd_conv tau_idemp_point)
      then show ?thesis
        using 1 2 by argo
    next
      case c
      then show ?thesis 
        by (metis d(1) d(5) fst_conv in_aff(1) in_aff(3) p_q_eqs(1) p_q_eqs(2) p_q_eqs(3) p_q_eqs(4) proj_add.simps(3) snd_conv tau_tau_d tau_tau_d')
    qed
    then show ?thesis 
      unfolding sum_eqs gluing_def
      apply(simp del: \<tau>.simps)
      ...
      using proj_add_aff[OF in_aff(1,3), of l1 t1] sum_eqs fst_conv apply force
      using proj_add_aff[OF in_aff(2,4), of l2 t2] sum_eqs fst_conv apply force
      using d(1) d(2) d(5) d(6) sum_eqs(2) by auto 
  qed
qed
...
proof -
  ...
  have e_proj: "\<langle>((x1, y1), 0)\<rangle> \<in> e_proj"
               "\<langle>((x2, y2), 0)\<rangle> \<in> e_proj"
  ...
  proof(cases)
    ...
    proof(cases)
      ...
      have eq1: "(?g1 \<oplus> (tf'' r ?g4)) \<oplus> ?g3 = ?g1"
        apply(rewrite remove_add_rot_comm) 
        using e_proj r_expr apply(simp,simp,simp)
        apply(rewrite proj_add_class_inv)
        using e_proj apply simp
        apply(rewrite remove_add_rot) 
        using e_proj r_expr apply(simp,simp,simp)
        apply(simp del: i.simps)
        apply(rewrite proj_add_class_identity)
        using e_proj apply simp 
        apply(rewrite get_sym[symmetric]) 
        using e_proj apply simp
        using e_proj_comp(2) r_expr(3) apply blast
        using r_expr apply simp
        apply(subst r_expr(3)[symmetric], subst g_expr')
        apply(rewrite comp_apply[symmetric, of g g'])
        apply(rewrite  g_expr'(3))
        by auto        
      show ?thesis
  ...
  next
    ...
    then show ?thesis 
      proof(cases)
        ...
        have "\<langle>((x1, y1), 0)\<rangle> \<oplus> \<langle>((x2, y2), 0)\<rangle> = ((tf r (\<langle>((1, 0), 0)\<rangle>)) \<oplus> \<langle>((x2, y2), 0)\<rangle>)" 
          using remove_rotations[OF one_in rot_expr(1)] rot_expr(2) by presburger
        also have "... = tf r (\<langle>((1, 0), 0)\<rangle> \<oplus> \<langle>((x2, y2), 0)\<rangle>)"  
          using remove_add_rotation assms rot_expr one_in by presburger
        also have "... = tf r (\<langle>((x2, y2), 0)\<rangle>)"
          using proj_add_class_identity 
          by (simp add: e_proj(2) identity_equiv)
        finally have eq1: "\<langle>((x1, y1), 0)\<rangle> \<oplus> \<langle>((x2, y2), 0)\<rangle> = tf r (\<langle>((x2, y2), 0)\<rangle>)" by argo

        have "(\<langle>((x1, y1), 0)\<rangle> \<oplus> \<langle>((x2, y2), 0)\<rangle>) \<oplus> \<langle>(i (x2, y2), 0)\<rangle> =
              (tf r (\<langle>((x2, y2), 0)\<rangle>) \<oplus> \<langle>(i (x2, y2), 0)\<rangle>)"
          using eq1 by argo
        also have "... = tf r (\<langle>((x2, y2), 0)\<rangle> \<oplus> \<langle>(i (x2, y2), 0)\<rangle>)"
          using remove_add_rotation rot_expr well_defined proj_addition_def assms one_in by simp
        also have "... = tf r (\<langle>((1, 0), 0)\<rangle>)"
          using assms identity_equiv proj_add_class_inv by auto   
        finally have eq2: "(\<langle>((x1, y1), 0)\<rangle> \<oplus> \<langle>((x2, y2), 0)\<rangle>) \<oplus> \<langle>(i (x2, y2), 0)\<rangle> =
                           tf r (\<langle>((1, 0), 0)\<rangle>)" by blast
        show ?thesis 
          apply(subst eq2)
          using remove_rotations[OF one_in rot_expr(1)] rot_expr(2) by presburger
      next
        ...
        have "\<langle>((x1, y1), 0)\<rangle> \<oplus> \<langle>((x2, y2), 0)\<rangle> = (\<langle>((x1, y1), 0)\<rangle> \<oplus> (tf r (\<langle>((1, 0), 0)\<rangle>)))" 
          using remove_rotations[OF one_in rot_expr(1)] rot_expr(2) by presburger
        also have "... = tf r (proj_addition (\<langle>((x1, y1), 0)\<rangle>) (\<langle>((1, 0), 0)\<rangle>))"  
        ...
        finally have eq1: "proj_addition (\<langle>((x1, y1), 0)\<rangle>) (\<langle>((x2, y2), 0)\<rangle>) =
                           tf r (\<langle>((x1, y1), 0)\<rangle>)" by argo

        have "proj_addition (proj_addition (\<langle>((x1, y1), 0)\<rangle>) (\<langle>((x2, y2), 0)\<rangle>)) (\<langle>(i (x2, y2), 0)\<rangle>) =
              proj_addition (tf r (\<langle>((x1, y1), 0)\<rangle>)) (\<langle>(i (x2, y2), 0)\<rangle>)"
          using eq1 by argo
        also have "... = tf r (proj_addition (\<langle>((x1, y1), 0)\<rangle>) (\<langle>(i (x2, y2), 0)\<rangle>))"
          using remove_add_rotation rot_expr well_defined proj_addition_def assms one_in by simp
        also have "... = tf r (proj_addition (\<langle>((x1, y1), 0)\<rangle>) (tf r' (gluing `` {(i (1, 0), 0)})))"
          using remove_rotations one_in rot_expr' by simp
        also have "... = tf r (tf r' (proj_addition (\<langle>((x1, y1), 0)\<rangle>) ((gluing `` {(i (1, 0), 0)}))))"
        ...
        also have "... = tf (id) (proj_addition (\<langle>((x1, y1), 0)\<rangle>) ((\<langle>((1, 0), 0)\<rangle>)))"
        ...
        finally have eq2: "proj_addition (proj_addition (\<langle>((x1, y1), 0)\<rangle>) (\<langle>((x2, y2), 0)\<rangle>)) (\<langle>(i (x2, y2), 0)\<rangle>) =
                           (\<langle>((x1, y1), 0)\<rangle>)" by blast
...
lemma assoc_1:
  assumes "\<langle>((x1, y1), 0)\<rangle>  \<in> e_proj" 
          "\<langle>((x2, y2), 0)\<rangle> \<in> e_proj" 
          "\<langle>((x3, y3), 0)\<rangle> \<in> e_proj"
  assumes a: "g \<in> symmetries" "(x2, y2) = (g \<circ> i) (x1, y1)"
  shows 
    "\<langle>((x1, y1), 0)\<rangle> \<oplus> \<langle>((x2, y2), 0)\<rangle> = tf'' (\<tau> \<circ> g) {((1,0),0)}" 
      (is "proj_addition ?g1 ?g2 = _")
    "(\<langle>((x1, y1), 0)\<rangle> \<oplus> \<langle>((x2, y2), 0)\<rangle>) \<oplus> \<langle>((x3, y3), 0)\<rangle> =
     tf'' (\<tau> \<circ> g) (\<langle>((x3, y3), 0)\<rangle>)" 
    "\<langle>((x1, y1), 0)\<rangle> \<oplus> (\<langle>((x2, y2), 0)\<rangle> \<oplus> \<langle>((x3, y3), 0)\<rangle>) =
     tf'' (\<tau> \<circ> g) (\<langle>((x3, y3), 0)\<rangle>)" (is "proj_addition ?g1 (proj_addition ?g2 ?g3) = _")
proof -
  ...
  have e_proj: "gluing `` {(g (i (x1, y1)), 0)}  \<in> e_proj"
               ...
               "proj_addition (gluing `` {(i (x1, y1), 0)}) (\<langle>((x3, y3), 0)\<rangle>) \<in> e_proj"
  ...
  also have "... = tf'' (\<tau> \<circ> g) (proj_addition ({((1,0),0)}) ?g3)"
    by (simp add: assms(3) one_in remove_add_rot rot)
  ...
  also have "... = tf'' (\<tau> \<circ> g)  (proj_addition ?g1 (proj_addition ?ig1 ?g3))"
    ...
    using e_proj(3) apply auto[1]
     apply (metis assms(3) e_proj(2) i.simps remove_add_rot rot 
...
lemma assoc_11:
  ...
  shows 
    "proj_addition (proj_addition (\<langle>((x1, y1), 0)\<rangle>) (\<langle>((x2, y2), 0)\<rangle>)) (\<langle>((x3, y3), 0)\<rangle>) = 
     proj_addition (\<langle>((x1, y1), 0)\<rangle>) (proj_addition (\<langle>((x2, y2), 0)\<rangle>) (\<langle>((x3, y3), 0)\<rangle>))"
...
proof -
  ...
  have e_proj: "gluing `` {(g (i (x2, y2)), 0)}  \<in> e_proj"
               "\<langle>(i (x2, y2), 0)\<rangle>  \<in> e_proj" (is "?ig2 \<in> _") 
...
proof -
  ...
  have e_proj_0: "\<langle>(i (x1,y1), 0)\<rangle> \<in> e_proj" (is "?ig1 \<in> _")
                 "\<langle>(i (x2,y2), 0)\<rangle> \<in> e_proj" (is "?ig2 \<in> _")
                 "\<langle>(i (x3,y3), 0)\<rangle> \<in> e_proj" (is "?ig3 \<in> _")
...
lemma assoc_with_zeros:
  assumes "\<langle>((x1, y1), 0)\<rangle>  \<in> e_proj" "\<langle>((x2, y2), 0)\<rangle> \<in> e_proj" 
          "\<langle>((x3, y3), 0)\<rangle> \<in> e_proj"
...
proof -
  ...
  next
    ...
    have add_in_1_2: "add_1_2 \<in> e'_aff"
      ...
      using add_closure_points e'_aff_def in_aff p_delta_1_2 by auto
    ...
    next
      ...
      have add_in: "add_2_3 \<in> e'_aff"
        unfolding add_2_3_def
        using add_closure_points e'_aff_def in_aff p_delta_2_3 by auto

      have e_proj_2_3: "\<langle>(add_2_3, 0)\<rangle> \<in> e_proj" 
                       "\<langle>(i add_2_3, 0)\<rangle> \<in> e_proj" 
        using add_in add_2_3_def e_proj_aff apply simp
        using add_in add_2_3_def e_proj_aff proj_add_class_inv by auto
      ...
      next
        ...
        proof(cases)
          ...
          have "(?g1 \<oplus> ?g2) \<oplus> ?g3 = (\<langle>(add_1_2, 0)\<rangle> \<oplus> \<langle>(g (i add_1_2), 0)\<rangle>)"
            using g_expr p_delta_1_2 gluing_add assms(1,2) add_1_2_def by force
          also have "... = tf'' (\<tau> \<circ> g) (\<langle>(add_1_2, 0)\<rangle> \<oplus> \<langle>(i add_1_2, 0)\<rangle>)" thm remove_add_rot
            using e_proj_1_2 g_expr assms remove_add_rot 
          also have "... = tf'' (\<tau> \<circ> g) ({((1, 0), 0)})"
            using e_proj_1_2 proj_add_class_inv by auto
          finally have eq1: "(?g1 \<oplus> ?g2) \<oplus> ?g3 = tf'' (\<tau> \<circ> g) ({((1, 0), 0)})" 
            by blast

          have add_1: "\<langle>((x1,y1),0)\<rangle> \<oplus> \<langle>((x2,y2),0)\<rangle> = \<langle>(add_1_2,0)\<rangle>"
            unfolding add_1_2_def 
            using e_proj_aff ext_curve_addition_axioms gluing_add in_aff(1) in_aff(2) p_delta_1_2(1) by fastforce
          have add_2: "?ig1 \<oplus> ?ig2 = \<langle>(i add_1_2,0)\<rangle>"
            using gluing_add[symmetric,of "fst (i (x1,y1))" "snd (i (x1,y1))" 0
                  ...
                  add_1_2_def inverse_rule_3 by auto

          have "?g1 \<oplus> (?g2 \<oplus> ?g3) = (?g1 \<oplus> (?g2 \<oplus> (\<langle>(g (i add_1_2), 0)\<rangle>)))"
            using g_expr by auto          
          also have "... =  (?g1 \<oplus> (tf'' (\<tau> \<circ> g) (\<langle>(i add_1_2,0)\<rangle> \<oplus> ?g2)))" 
            apply(subst remove_add_sym[of "(x2,y2)" 0 "i add_1_2" 0 g])
            using assms(2,3) g_expr(1,2) e_proj_1_2 proj_addition_comm by auto
          also have "... = (?g1 \<oplus> (tf'' (\<tau> \<circ> g) ((?ig1 \<oplus> ?ig2) \<oplus> ?g2)))"
            using add_2 by simp
        
          also have "... = proj_addition ?g1 (tf'' (\<tau> \<circ> g) ?ig1)"
            thm cancellation_assoc[of ]
    ...
    next
      ...
      next
        ...
        proof(cases)
          ...
          also have "... = tf'' (\<tau> \<circ> g) {((1, 0), 0)}"
            using assms(1) proj_add_class_comm proj_addition_def proj_add_class_inv by auto
          finally have eq2: "proj_addition (\<langle>((x1, y1), 0)\<rangle>)
                              (proj_addition (\<langle>((x2, y2), 0)\<rangle>) (\<langle>((x3, y3), 0)\<rangle>)) = 
                        tf'' (\<tau> \<circ> g) {((1, 0), 0)}" by blast
          then show ?thesis using eq1 eq2 by blast
        next
          case 2222
...
proof -
  let ?t1 = "(proj_addition (proj_addition (\<langle>((x1, y1), 0)\<rangle>) (\<langle>((x2, y2), 0)\<rangle>))
                                      (\<langle>((x3, y3), 0)\<rangle>))"
  let ?t2 = "proj_addition (\<langle>((x1, y1), 0)\<rangle>)
              (proj_addition (\<langle>((x2, y2), 0)\<rangle>) (\<langle>((x3, y3), 0)\<rangle>))"
  
  have e_proj_0: "\<langle>((x1, y1), 0)\<rangle> \<in> e_proj"
                 "\<langle>((x2, y2), 0)\<rangle> \<in> e_proj"
                 "\<langle>((x3, y3), 0)\<rangle> \<in> e_proj"
    ...
    using assms e_proj_aff by blast+
  have e_proj_add_0: "proj_addition (\<langle>((x1, y1), 0)\<rangle>) (\<langle>((x2, y2), 0)\<rangle>) \<in> e_proj" 
                     "proj_addition (\<langle>((x2, y2), 0)\<rangle>) (\<langle>((x3, y3), 0)\<rangle>) \<in> e_proj"
                     "proj_addition (\<langle>((x2, y2), 0)\<rangle>) (gluing `` {((x3, y3), 1)}) \<in> e_proj"
                     "proj_addition (\<langle>((x1, y1), 0)\<rangle>) (gluing `` {((x2, y2), 1)}) \<in> e_proj" 
                     "proj_addition (gluing `` {((x2, y2), 1)}) (\<langle>((x3, y3), 0)\<rangle>) \<in> e_proj"
  ...
  proof(cases "l = 0")
    ...
    proof(cases "m = 0")
      ...
      next
        ...
        have eq1: "proj_addition (proj_addition (\<langle>((x1, y1), 0)\<rangle>) (\<langle>((x2, y2), 0)\<rangle>))
        ...
        have eq2: "proj_addition (\<langle>((x1, y1), 0)\<rangle>)
                            (proj_addition (\<langle>((x2, y2), 0)\<rangle>) (gluing `` {((x3, y3), 1)})) =
               tf'(?t2)"
          apply(subst tf_tau[of _ _ 0,simplified],simp add: e_proj_0)
    ...
    next
      ...
      proof(cases "n = 0")
        ...
        have eq1: "proj_addition (proj_addition (\<langle>((x1, y1), 0)\<rangle>) (gluing `` {((x2, y2), 1)}))
                                 (\<langle>((x3, y3), 0)\<rangle>) = tf'(?t1)"
          apply(subst tf_tau[of _ _ 0,simplified],simp add: e_proj_0)
          apply(subst remove_add_tau',(simp add: e_proj_0)+)
          by(subst remove_add_tau,(simp add: e_proj_0 e_proj_add_0)+)
        have eq2: "proj_addition (\<langle>((x1, y1), 0)\<rangle>)
                    (proj_addition (gluing `` {((x2, y2), 1)}) (\<langle>((x3, y3), 0)\<rangle>)) = 
                  tf'(?t2)"
          apply(subst tf_tau[of _ _ 0,simplified],simp add: e_proj_0)
          apply(subst remove_add_tau,(simp add: e_proj_0)+)
          by(subst remove_add_tau',(simp add: e_proj_0 e_proj_add_0)+)
  ...
  next
    ...
    proof(cases "m = 0")
      ...
      proof(cases "n = 0")
        ...
        have eq1: "proj_addition (proj_addition (gluing `` {((x1, y1), 1)}) (\<langle>((x2, y2), 0)\<rangle>))
                        (\<langle>((x3, y3), 0)\<rangle>) = tf'(?t1)"
          apply(subst tf_tau[of _ _ 0,simplified],simp add: e_proj_0)
          apply(subst remove_add_tau,(simp add: e_proj_0)+)
          by(subst remove_add_tau,(simp add: e_proj_0 e_proj_add_0)+)
