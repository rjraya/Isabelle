...
ML \<open>
(* combinations is a list of lists and atoms_list is a list *)
fun combine_atoms atoms_list combinations = 
  let 
    fun comb li xi = li @ (single xi)
    
    fun for_each_xi xi =
       (map (fn li => comb li xi) combinations)
  in
   List.concat(map for_each_xi atoms_list)
  end;

fun combine_until_size size atoms = 
 if size = 0 then atoms
 else 
...
ML \<open>
val atoms = [@{term "2::'a"},@{term "x0::'a"}, @{term "x1::'a"}, @{term "y0::'a"}, @{term "y1::'a"}]

fun combine_atoms_rec

fun combine_atoms i l =
  if i == 0 then [] else combine_atoms_rec i
...
let 
val ctxt = @{context}
val pol = @{term "(y0::'a)^2 - x1^2"};
val pols = @{term "q1*(-1 + x0^2 + y0^2 - t^2 * x0^2 * y0^2) +
              q2*(-1 + x1^2 + y1^2 - t^2 * x1^2 * y1^2) +
              q3*(x0 * y0 + x1 * y1) +
              q4*(x0 * y1 + x1 * y0)"}
val list = [@{term "2::'a"}, @{term "t::'a"}, @{term "(1-t^2)::'a"}, @{term "x0::'a"}, @{term "x1::'a"}, @{term "y0::'a"}, @{term "y1::'a"},
            @{term "2*t::'a"},@{term "2*(1-t^2)::'a"},@{term "2*x0::'a"},@{term "2*x1::'a"},@{term "2*y0::'a"},@{term "2*y1::'a"},
            @{term "x0*y0::'a"},@{term "x1*y1::'a"},@{term "2*x0::'a"},@{term "2*x1::'a"},@{term "2*y0::'a"},@{term "2*y1::'a"},
            @{term "2*x0*y0::'a"} ]


fun f l = 
  let
    val poly = HOLogic.mk_binop "Groups.times_class.times" (l, pol)
    val eq = HOLogic.mk_Trueprop( 
              HOLogic.mk_exists ("q1",@{typ 'a},
               HOLogic.mk_exists ("q2",@{typ 'a},
                HOLogic.mk_exists ("q3",@{typ 'a},
                 HOLogic.mk_exists ("q4",@{typ 'a},
                  HOLogic.mk_eq(poly,pols))))))
  in 
    Goal.prove ctxt [] [] eq 
      (fn _ => Groebner.algebra_tac [] [] ctxt 1)
    handle Option => let val _ = @{print} "True" in @{thm HOL.TrueI} end
  end;
in
map f list
end
  ...
  next
    ...
    next
      ...
      have zeros: "a1 * b0 + b1 * a0 = 0" "a0 * b0 + a1 * b1 = 0" 
        using 3 \<delta>_minus_expr p\<delta>'_expr
        by auto
  ...
  next
    ...
    proof(cases "delta_minus a1 b1 (fst q) (snd q) = 0")
      ...
      proof(cases "\<delta>_plus a0 b0 = 0")
        ...
        have "\<exists> q1 q2 q3 q4. 
          a0*b0 - a1*b1 = q1 * \<delta>' a0 b0 + q3 * \<delta>_plus a0 b0"
          unfolding \<delta>'_expr \<delta>_plus_expr \<delta>_minus_expr
          using True \<delta>_plus_expr by auto

        have eq1': "a0*b0 - a1*b1 = 0" 
          using \<delta>'_expr q_def tau_sq tq_expr cas1(1) 
        ...
        have eq2: "b0^2 - a1^2 = 0"
          using cas1(2) eq1 a0_nz a1_nz unfolding \<delta>_plus_expr
          apply(simp add: field_simps) 
          by algebra
    ...
    next
      ...
      proof(cases "\<delta>_minus a0 b0 = 0")
        ...
        have eq1': "a0 * b0 + a1 * b1 = 0"
          using p\<delta>'_expr cas1(1) p\<delta>'_def q_def tau_sq tq_expr by auto 
        then have eq1: "a0 = - (a1 * b1) / b0"  
          using a0_nz(2) 
          by(simp add: divide_simps,algebra) 
      ...
      next
        ...
        then have eq1: "a0*b0 + a1*b1 = 0" 
          using p\<delta>'_def p\<delta>'_expr tq_expr q_def tau_sq cas2(1) by force
