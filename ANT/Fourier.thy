...
proof -
 {fix l
  ...
  have period: "periodic (\<lambda>x. unity_root k x) (k*l)" by simp}
  note period = this
 {fix n l
  have "unity_root k (-(n+k)*l) = inverse (unity_root k ((n+k)*l))"
    by(subst mult_minus_left,
       subst SMT.int_ops(7)[symmetric],
       subst unity_minus,blast) 
  also have "... = inverse (unity_root k (n*l))"
    apply(simp add: distrib_right)
    apply(subst SMT.int_ops(7)[symmetric])+
    apply(subst SMT.int_ops(5)[symmetric])
    using period unfolding periodic_def by blast
  also have "... = unity_root k (-n*l)"
    using unity_minus by simp 
  ...
  proof - 
    ...
    then have p_s: "\<forall>i<k. periodic (\<lambda> n. g(n)* unity_root k (i * int n)) k"
      by simp
    have "periodic (\<lambda>i. \<Sum>n<k. g(n)* unity_root k (i * int n)) k"
      unfolding periodic_def
    proof
      fix n
...
lemma fourier_uniqueness:
  fixes f g :: "nat \<Rightarrow> complex" 
  assumes "k > 0"
  assumes "periodic f k" "periodic g k" (*periodicity of g required by theorem? *)
...
lemma I_left_neutral:
 "dirichlet_prod I f n = f n " 
 if "n > 0" for f :: "nat \<Rightarrow> complex" and n
  using I_right_neutral[OF that, of f] 
        dirichlet_prod_commutes[of f I]
  by argo
...
next
  ...
  have 2: "unity_root n n = 1" 
    using unity_periodic[of n] add.left_neutral
  proof -
    have "1 = unity_root n (int 0)"
       using 1 by auto
    also have "unity_root n (int 0) = unity_root n (int (0 + n))"
      using unity_periodic[of n] periodic_def by algebra
...
proof -
  ...
  proof -
    {fix d c
    assume "d dvd k"
    have "unity_root k (- m * (c * d)) = unity_root (k div d) (- m * c)"
      apply(subst mult_minus_left)+
      apply(subst SMT.int_ops(7)[symmetric])+
      apply(subst mult.assoc[symmetric])
      apply(subst SMT.int_ops(7))
      apply(subst unity_minus)+
      apply(subst unity_div[OF \<open>d dvd k\<close>]) 
      by blast}
    then show ?thesis by simp
  qed
  also have "... = 1 / k *
  ...
  proof -
    ...
    proof(intro sum.mono_neutral_right,simp add: \<open>k > 0\<close>,blast,standard)
      ...
      show "geometric_sum i (- int m) * (f (k div i) * g i) = 0" 
        using geometric_sum(1)[OF \<open>i \<ge> 1\<close> \<open>\<not> i dvd (-m)\<close>] by simp
    qed   
  ...
  proof -    
    ...
    proof -
      ...
      moreover have "periodic (\<lambda> m. unity_root k (int m * n)) k" 
        using unity_periodic_mult[of k n]
...
proof -
  ...
  also have "... = f(m div d1) * f(k div d2)"
    by (simp add: \<open>d1 dvd m\<close> assms(1) completely_multiplicative_function.mult)
  finally show ?thesis by simp
qed
...
proof -
  ...
  have "s f g (m*k) (a*b) = 
    ...
    unfolding s_def by(rule sum.cong, simp add: gcd.commute,blast) 
  also have "... = 
  ...
  also have "... = (\<Sum> d1 | d1 dvd gcd a m.
    ...
    also have "... = (\<Sum> d1 | d1 dvd gcd a m.
     (\<Sum> d2 |  d2 dvd gcd k b. 
      ...
      using assms(5) assms(8) multiplicative_function.mult_coprime by fastforce
    also have "... = (\<Sum> d1 | d1 dvd gcd a m.
     (\<Sum> d2 |  d2 dvd gcd k b. 
...
proof -
  ...
  proof -
    have "?a = (\<Sum> d | d dvd a \<and> coprime N d. f(a div d) * moebius_mu (N*d) * h (N*d))"
      by(rule sum.mono_neutral_right)(auto simp add: moebius_not_c 3)
  ...
  proof -
    ...
    then have eq2: "(\<Prod>p\<in>{p. p \<in>prime_factors a \<and> \<not> p dvd N}. 1 - h p / f p) = 
      ...
      by auto
    also have eq: "... = prod (\<lambda> p. 1 - h p / f p) {p. p \<in>prime_factors (a*N)} div
  ...
  proof -
    ...
    have "F(k) = (f k) * (\<Prod>p\<in> prime_factors k. 1 - (h p div f p))"
      unfolding F_def g_def by(intro F_form)(auto simp add: assms(4-7))
...
lemma e_characterization:
  assumes "x > (0::real)"
  shows   "exp 1 > (1 + 1 / x) powr x" and 
...
proof -   
  ...
  from e_characterization(2)[OF \<open>k-1 > 0\<close>]
  ...
  proof -
    from ln_less_cancel_iff[of "exp(1)",simplified,OF \<open>(1 + 1 / (k - 1)) powr k > 0\<close>]
        e_characterization[OF \<open>k - 1 > 0\<close>]
    show ?thesis by simp
  qed
