...
(* array_assign is automatically proven *)
...
(*
 ...
 results in modifying, with the command a[j] ::= x[i], cells that would not be 
 modified otherwise. 
...
(* 
 ...
 that the indexes i,j are the same in states s,s'. We prove each implication
 separately and combine the resulting lemma into a theorem:
*)
...
(*
 ...
 We now try to give conditions on the structure of the term so that we can 
 ensure the interesting condition above, namely, theorem sufficient1.

 A way to ensure this is to guarantee that the array x is not accessed in the 
 derivation tree of i and j. The following function looks for an access to x[i]
 expressions to x[i]:
*)
...
definition no_ref where
"no_ref x j \<longleftrightarrow> (\<forall> l. \<not> syntactic_subterm (Vidx x l) j)"

lemma imp_sufficient1: "
  no_ref x j \<Longrightarrow>
  (x[i] ::= a, s) \<Rightarrow> s' \<Longrightarrow>
  aval j s = aval j s' 
"
  apply(simp add: no_ref_def, erule AssignIdxE)
  by(induction "j", auto)

(*
 The theorem in this case says that, if we don't have any reference to x in 
 the derivation trees of i and j, the commands are equivalent.
*)

theorem syntactic_sufficient1: "
       no_ref x j \<Longrightarrow>
       no_ref x i \<Longrightarrow>
       (x[i] ::= Vidx a j;; a[j] ::= Vidx x i) \<sim> (x[i] ::= Vidx a j)
"
  by (simp add: imp_sufficient1 sufficient1)

(*
 Semantic and syntactic criterion:
 ...
 only accesses at (semantic) position i, are disallowed.

 Notice that the semantic part is important. If we don't do it like this there
 could be terms that get evaluated to x[i] and that may affect to the values of
 i or j. We would need special conditions in the memory state as above.
*)
...
definition no_ref_i where
"no_ref_i x j s i \<longleftrightarrow> (\<not> semantic_subterm (Vidx x i) j s)"

lemma aux_imp_sufficient2: "
  no_ref_i x j s i \<Longrightarrow>
  (x[i] ::= a, s) \<Rightarrow> s' \<Longrightarrow>
  aval j s = aval j s' 
" 
  apply(simp add: no_ref_i_def, erule AssignIdxE)
  by(induction "j",auto)

theorem semantic_sufficient: "
  (\<forall> s. no_ref_i x j s i \<and> no_ref_i x i s i) \<Longrightarrow>
...
program_spec substring_step
  ...
  ensures "0 \<le> j \<and> j \<le> len \<and> i+j \<le> h \<and> 
           lran a i (i+j) = lran b 0 j \<and>
...
while(i < h)
 @invariant \<open>
   l \<le> i \<and> i \<le> h \<and> 0 \<le> j \<and> j \<le> len \<and> 
   (match = 0 \<and> \<not> match = 1 \<or> match = 1 \<and> \<not> match = 0) \<and>
