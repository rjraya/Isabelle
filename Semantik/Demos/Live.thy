...
corollary bury_sim: "bury c UNIV \<sim> c" unfolding equiv_c_def
  by(metis final_bury_correct final_bury_correct2)


consts while :: "('a \<Rightarrow> bool) \<Rightarrow> ('a \<Rightarrow> 'a) \<Rightarrow> 'a \<Rightarrow> 'a"
lemma while_unfold: "while b c s = (if b s then while b c (c s) else s)"
  sorry 
...
proof(induction s0) 
 ...
 then show ?case
  ...
  using less.prems CONS by blast
qed
