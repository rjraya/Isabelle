...
lemma alpha_illustration:
  ...
  and   "x \<sharp> T \<Longrightarrow> y \<sharp> T \<Longrightarrow> (\<lambda>x:T. Var x) = (\<lambda>y:T. Var y)"
  by(simp_all add: ty.inject trm.inject ltrm.inject alpha calc_atm fresh_atm perm_fresh_fresh)
...
TODO: variable permutation does not leave type invariant

lemma ty_vrs_prm_simp:
  fixes pi::"vrs prm"
  and   S::"ty"
  shows "pi\<bullet>S = S" 
  apply(induct S rule: ty_aty.induct[THEN conjunct1]) 
...
TODO: variables may appear in types
