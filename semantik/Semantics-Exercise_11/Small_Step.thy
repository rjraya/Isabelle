...
Until:   "(DO c UNTIL b,s) \<rightarrow>
            (c;; IF b THEN SKIP ELSE DO c UNTIL b,s)"
...
next
  ...
  note [intro] = star_seq2 seq_comp
  have "(DO c UNTIL b, s) \<rightarrow> (c;; IF b THEN SKIP ELSE DO c UNTIL b, s)" ..
  also have "\<dots> \<rightarrow>* (SKIP;; IF b THEN SKIP ELSE DO c UNTIL b, s')"
    using \<open>(c, s) \<rightarrow>* (SKIP, s')\<close> by blast
  also have "\<dots> \<rightarrow>* (SKIP, s')"
    using \<open>bval _ _\<close> by blast
  finally show ?case .
next
  case (UntilFalse c s s' b t)
  note [intro] = star_seq2 seq_comp star_trans
  have If: "(IF b THEN SKIP ELSE DO c UNTIL b, s') \<rightarrow>* (DO c UNTIL b, s')"
    using \<open>\<not> bval b s'\<close> by blast
  have "(DO c UNTIL b, s) \<rightarrow> (c;; IF b THEN SKIP ELSE DO c UNTIL b, s)" ..
  also have "\<dots> \<rightarrow>* (SKIP;; IF b THEN SKIP ELSE DO c UNTIL b, s')"
    using \<open>(c, s) \<rightarrow>* (SKIP, s')\<close> by blast
  also have "\<dots> \<rightarrow>* (SKIP, t)"
    using UntilFalse.IH If by blast
  finally show ?case .
qed
