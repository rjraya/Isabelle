...
theory Compiler imports Big_Step Small_Step
begin
...
where
  "exec P \<equiv> (exec1 P)\<^sup>*\<^sup>*"

lemmas exec_induct = star_induct [of "exec1 P", split_format(complete)]
...
lemma exec_appendR: "P \<turnstile> c \<rightarrow>* c' \<Longrightarrow> P@P' \<turnstile> c \<rightarrow>* c'"
  by (induction rule: star_induct1) (fastforce intro: star_step exec1_appendR)+
...
lemma exec_appendL:
 ...
 "P \<turnstile> (i,s,stk) \<rightarrow>* (i',s',stk')  \<Longrightarrow>
  P' @ P \<turnstile> (size(P')+i,s,stk) \<rightarrow>* (size(P')+i',s',stk')"
  by (induction rule: exec_induct) (blast intro: star_step exec1_appendL)+
...
next
  case (Less a1 a2) thus ?case  
    by (fastforce intro!: acomp_correct)
qed fastforce    
...
next
  case (IfFalse b s c2 t c1) thus ?case by (force intro!: bcomp_correct)
next
