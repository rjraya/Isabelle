...
section \<open>Proving Partial Correctness\<close>
  ...
  definition "prog_min \<equiv> IF (Less (V ''x'') (V ''y'')) THEN ''z'' ::= V ''x'' ELSE ''z'' ::= V ''y''"
  ...
  lemma "s ''n'' \<ge> 0 \<Longrightarrow> wlp prog_exp (\<lambda>s'. s' ''a'' = 2^nat (s ''n'')) s"  
    ...
    apply (subst wlp_assign_eq)
    apply simp
    apply (clarsimp simp: nat_distribs algebra_simps)
    apply simp
    done
  ...
  lemma "s ''n'' \<ge> 0 \<Longrightarrow> wlp prog_exp (\<lambda>s'. s' ''a'' = 2^nat (s ''n'')) s"  
    ...
    (* As a nice side effect, the simplifier gets rid of trivial goals altogether *)    
...
lemma wp_untilI':
  ...
  text \<open>Equivalent form of if rule, with splitting\<close>
  lemma wp_ifI[vcg_rules]: "\<lbrakk>bval b s \<Longrightarrow> wp c\<^sub>1 Q s; \<not>bval b s \<Longrightarrow> wp c\<^sub>2 Q s\<rbrakk> \<Longrightarrow> wp (IF b THEN c\<^sub>1 ELSE c\<^sub>2) Q s"
    by (simp add: wp_eq')
  ...
  lemma "s ''n'' \<ge> 0 \<Longrightarrow> wp prog_exp (\<lambda>s'. s' ''a'' = 2^nat (s ''n'')) s"  
    ...
    apply (subst wp_annotate_while[where 
          I="\<lambda>s'. s' ''a'' = 2 ^ nat (s ''n'' - s' ''n'') \<and> 0 \<le> s' ''n'' \<and> s' ''n'' \<le> s ''n''" 
      and R = "measure (\<lambda>s. nat (s ''n''))"]
      )
    apply smartvcg  
    by (auto simp: algebra_simps nat_distribs) 
